...
let rec gen ~max_depth ~max_breadth ~max_file_sz () : filesystem list * int =
  ...
  if num_obj = 0 then [], 0
  (* Empty directory if max depth hit *)
    ...
    let rec aux cnt acc sz =
      if cnt >= num_obj then acc, sz
        ...
        let file_sz = Random.int max_file_sz in
...
let () =
  let max_depth = try Sys.argv.(1) |> int_of_string with _ -> 15 in
  let max_breadth = try Sys.argv.(2) |> int_of_string with _ -> 1000 in
  ...
  Format.printf "\n\nThis will require %d MB of space Are you sure you want to generate this? (y or n) " mb_sz;
